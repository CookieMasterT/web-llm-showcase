...
function updateProbabilitiesChart(topLogprobs, chosenTokenText) {
    ...
    const pickChances = adjusted.map(item => {
        ...
        const chance = (inP && finalSum > 0) ? (item.temp_prob / finalSum) * 100 : 0.0;
    ...
    withRawChance.forEach((item) => {
        ...
        const isChosen = chosenTokenText && (
            item.token === chosenTokenText ||
            item.token.replace(/[^a-zA-Z0-9]/g, '') === chosenTokenText.replace(/[^a-zA-Z0-9]/g, '')
        );
        if (isChosen) {
...
async function streamingGenerating(messages, onUpdate, onFinish, onError) {
    ...
    try {
        ...
        for await (const chunk of completion) {
            ...
            const speedDelay = parseInt(document.getElementById("speed-slider").value);
            if (speedDelay > 0) {
            ...
            if (logprobsObj && logprobsObj.content && logprobsObj.content.length > 0) {
...
/*************** Model Selection Search & Initialization ***************/
...
document.getElementById("reset-btn").addEventListener("click", function () {
    ...
    const chatBox = document.getElementById("chat-box");
    chatBox.innerHTML = '<div class="system-prompt-box">System Prompt: You are a helpful AI agent helping users.</div>';
    ...
    document.getElementById("chosen-tokens-display").innerHTML = "As text is generated here, new tokens appear...";
    document.getElementById("probabilities-chart").innerHTML = "";
    document.getElementById("user-input").value = "";
    document.getElementById("user-input").setAttribute("placeholder", "Send a message...");
    document.getElementById("send").disabled = false;
});
